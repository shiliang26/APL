...
class EmbeddingManager(nn.Module):
        ...
        for placeholder_string, placeholder_token in self.string_to_token_dict.items():
            ...
            if self.max_vectors_per_token == 1: # If there's only one vector per token, we can do a simple replacement
                ...
                for idx in range(len(sorted_rows)):
                    ...
                    new_token_row = torch.cat([tokenized_text[row][:col], placeholder_token.repeat(num_vectors_for_token), tokenized_text[row][col + 1:]], axis=0)[:n]
    ...
    def load(self, ckpt_path):
        ...
        for key in ckpt['string_to_param'].keys():
            self.string_to_param_dict[key] = torch.nn.Parameter(ckpt['string_to_param'][key])
            self.max_vectors_per_token = ckpt['string_to_param'][key].shape[0]
